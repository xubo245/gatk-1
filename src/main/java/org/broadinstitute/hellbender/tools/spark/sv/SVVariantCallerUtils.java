...
final class SVVariantCallerUtils {
     ...
     *
     * @throws IllegalArgumentException if fails check by {@link #checkCigarElements(List)}
     */
    @VisibleForTesting
    static int getNumHardClippingBases(final boolean fromStart, final List<CigarElement> cigarElements) {

        checkCigarElements(cigarElements);
     ...
     *     <li>when having only 1 operations, it must be an alignment;</li>
     ...
     */
    @VisibleForTesting
    static void checkCigarElements(final List<CigarElement> cigarElements) {
        Utils.validateArg(!cigarElements.isEmpty(), "Cannot parse empty list cigarElements");
        Utils.validateArg(cigarElements.size()>1 || (cigarElements.size()==1 && cigarElements.get(0).getOperator().isAlignment()),
                "invalid input cigar elements " + cigarElements.toString());
        Utils.validateArg ( !(cigarElements.get(0).getOperator().isClipping() && cigarElements.get(1).getOperator()==CigarOperator.D)
                        &&
                        !(cigarElements.get(cigarElements.size()-1).getOperator().isClipping() && cigarElements.get(cigarElements.size()-2).getOperator()==CigarOperator.D),
