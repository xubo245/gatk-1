...
public class SVVariantConsensusCallUnitTest extends BaseTest {
    ...
    private static void seeIfItWorks_typeInference(final NovelAdjacencyReferenceLocations breakpoints,
        ...
        final SVTYPE svtype = SVVariantConsensusCall.getType(breakpoints);
        Assert.assertEquals(svtype.toString(), expectedTypeString);

        final Set<String> flags = SVVariantConsensusCall.setFlags(breakpoints, svtype).keySet();
    ...
    public void testGetType() throws IOException {
        ...
        seeIfItWorks_typeInference(breakpoints, SVTYPE.RECOGNIZED_SVTYPES.INV.name(), ImmutableSet.of(GATKSVVCFHeaderLines.INV33));

        breakpoints = SVCallerTestDataProvider.forSimpleInversionWithHom_leftPlus._3();
        seeIfItWorks_typeInference(breakpoints, SVTYPE.RECOGNIZED_SVTYPES.INV.name(), ImmutableSet.of(GATKSVVCFHeaderLines.INV55));

        // simple deletion
        breakpoints = SVCallerTestDataProvider.forSimpleDeletion_plus._3();
        seeIfItWorks_typeInference(breakpoints, SVTYPE.RECOGNIZED_SVTYPES.DEL.name(), Collections.emptySet());

        // simple insertion
        breakpoints = SVCallerTestDataProvider.forSimpleInsertion_minus._3();
        seeIfItWorks_typeInference(breakpoints, SVTYPE.RECOGNIZED_SVTYPES.INS.name(), Collections.emptySet());
        ...
        seeIfItWorks_typeInference(breakpoints, SVTYPE.RECOGNIZED_SVTYPES.DEL.name(), ImmutableSet.of(SVConstants.CallingStepConstants.TANDUP_CONTRACTION_STRING));

        // simple tandem dup expansion from 1 unit to 2 units
        breakpoints = SVCallerTestDataProvider.forSimpleTanDupExpansion_minus._3();
        seeIfItWorks_typeInference(breakpoints, SVTYPE.RECOGNIZED_SVTYPES.DUP.name(), ImmutableSet.of(SVConstants.CallingStepConstants.TANDUP_EXPANSION_STRING));
